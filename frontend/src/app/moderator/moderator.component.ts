...
export class ModeratorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.notifications=this.moderator.obavestenja;
      ...
      if(booksFromDB!=null){
        ...
        this.loanService.findByUser(this.moderator.korisnicko_ime).subscribe((loansFromDB: Loan[])=>{
          if(loansFromDB!=null) {
            ...
            if(this.moderator.blokiran) {this.notifications.push("You have been blocked by the administrator!")}
            for (const n of this.moderator.obavestenja) {
              if (n.includes('reserved')) {
                this.userService.unnotify(this.moderator.korisnicko_ime,'reserved').subscribe(resp=>{

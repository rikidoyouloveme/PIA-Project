...
export class AdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.notifications=this.admin.obavestenja;
    ...
    this.bookService.findAll().subscribe((booksFromDB: Book[])=>{
      if(booksFromDB!=null){
        ...
        this.loanService.findByUser(this.admin.korisnicko_ime).subscribe((loansFromDB: Loan[])=>{
          if(loansFromDB!=null) {
            let broj = 0;
            ...
            if(this.admin.blokiran) {this.notifications.push("You have been blocked by the administrator!")}
            for (const n of this.admin.obavestenja) {
              if (n.includes('reserved')) {
                this.userService.unnotify(this.admin.korisnicko_ime,'reserved').subscribe(resp=>{
                  this.message = resp['message'];
                  this.userService.login(this.admin.korisnicko_ime,this.admin.lozinka).subscribe((userFromDB: User)=>{
                    if(userFromDB!=null){
